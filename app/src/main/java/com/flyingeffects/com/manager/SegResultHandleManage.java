 ...
 * 抠像结果处理工具类
 */
public class SegResultHandleManage {
     ...
     * @return
     */
    public static Bitmap setBlackWhite(Bitmap image, byte[] alphas) {
        ...
        for (int i = 0; i < (temp.length / 4); i++) {
            pixels[i * 4 + 0] = alphas[i];       //R
            pixels[i * 4 + 1] = alphas[i];       //G
            pixels[i * 4 + 2] = alphas[i];       //B
     ...
     * @return
     */
    public static Bitmap backGroudBlend(Bitmap fgBitmap, Bitmap bgBitmap, byte[] alphas) {
        ...
        for (int i = 0; i < fgHeight; i++) {
            for (int j = 0; j < fgWidth; j++) {
                ...
                float alpha = (float) (alphas[dstIndex] & 0xFF);
                pixels[fgIndex + 0] = (byte) (fgR * (alpha / 255.0f) + (bgR * (1 - alpha / 255.0f)));       //R
                pixels[fgIndex + 1] = (byte) (fgG * (alpha / 255.0f) + (bgG * (1 - alpha / 255.0f)));       //G
                pixels[fgIndex + 2] = (byte) (fgB * (alpha / 255.0f) + (bgB * (1 - alpha / 255.0f)));       //B
