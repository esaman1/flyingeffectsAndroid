...
Java_com_megvii_segjni_SegJni_nativeCreateSegHandler(JNIEnv *env, jclass type, jobject context,
                                                     jbyteArray segmentModel_, jint thread_count) {
    ...
    retCode = mg_seg.CreateApiHandle((const MG_BYTE *) segmentModel, (const MG_INT32) model_len,
                                     thread_count, seg_handler);
    if (retCode != 0) {
...
JNIEXPORT jint JNICALL
Java_com_megvii_segjni_SegJni_nativeSegImage(JNIEnv *env, jclass type, jbyteArray data_, jint width,
                                             jint height, jbyteArray output_,
                                             jboolean segment_flag) {
    ...
    if (seg_handler != nullptr) {
        ...
        int outWidth;
        int outHeight;
        int ret = mg_seg.SegmentImage(seg_handler, (const MG_BYTE *) data, width, height,
                                      MG_IMAGEMODE_RGBA, config, &out, &outWidth, &outHeight,
                                      &out_mode, false);
        if (ret != 0 || out == NULL) {
...
Java_com_megvii_segjni_SegJni_nativeSegCamera(JNIEnv *env, jclass type, jbyteArray data_,
                                              jint width, jint height, jint rotate, jint radius,
                                              jint fps, jintArray segSize_) {
    jbyte *data = env->GetByteArrayElements(data_, NULL);
    ...
    if (seg_handler != nullptr) {
        ...
        int outHeight;
        int ret = mg_seg.SegmentImage(seg_handler, (const MG_BYTE *) data, width, height,
                                      MG_IMAGEMODE_NV21, config, &out, &outWidth, &outHeight,
                                      &out_mode, true);
...
JNIEXPORT jint JNICALL
Java_com_megvii_segjni_SegJni_nativeBlendImage(JNIEnv *env, jclass type, jobject jObjSrcImg,
                                               jbyteArray jObjAlpha) {
            ...
            if (lpSegMask != nullptr) {
                ...
                for (int i = 0; i < info.height; i++) {
                    ...
                    for (int j = 0; j < info.width; j++) {
                        ...
                        {
                            *lpTmpDst = (uint8_t) (0.0f * (255.0f - *lpAlpha) / 255.0f +
...
JNIEXPORT jint JNICALL
Java_com_megvii_segjni_SegJni_nativeBlendBackGroud(JNIEnv *env, jclass type, jobject jObjfgImg,
                                                   jobject jObjbgImg, jbyteArray jObjAlpha) {
    AndroidBitmapInfo fgImageInfo;
    memset(&fgImageInfo, 0, sizeof(AndroidBitmapInfo));
    AndroidBitmap_getInfo(env, jObjfgImg, &fgImageInfo);

    AndroidBitmapInfo bgImageInfo;
    memset(&bgImageInfo, 0, sizeof(AndroidBitmapInfo));
    AndroidBitmap_getInfo(env, jObjbgImg, &bgImageInfo);

    if (fgImageInfo.width > 0 && fgImageInfo.height > 0) {
        uint8_t *lpSrcdata = nullptr;
        AndroidBitmap_lockPixels(env, jObjfgImg, (void **) &lpSrcdata);

        uint8_t *lpbgdata = nullptr;
        AndroidBitmap_lockPixels(env, jObjbgImg, (void **) &lpbgdata);

        if (lpSrcdata != nullptr && lpbgdata != nullptr) {
            uint8_t *lpSegMask = (uint8_t *) env->GetByteArrayElements(jObjAlpha, nullptr);
            if (lpSegMask != nullptr) {
                uint8_t *lpAlpha = lpSegMask;
                for (int i = 0; i < fgImageInfo.height; i++) {
                    uint8_t *lpTmpDst = lpSrcdata;
                    uint8_t *lpbgTmpDst = lpbgdata;
                    for (int j = 0; j < fgImageInfo.width; j++) {

                        {
                            *lpTmpDst = (uint8_t) (*lpbgTmpDst * (255.0f - *lpAlpha) / 255.0f +
                                                   (*lpTmpDst * *lpAlpha) / 255.0f);
                            lpTmpDst++;
                            lpbgTmpDst++;
                            *lpTmpDst = (uint8_t) (*lpbgTmpDst * (255.0f - *lpAlpha) / 255.0f +
                    ...
                    lpSrcdata += fgImageInfo.stride;
                    lpbgdata += bgImageInfo.stride;
                }
                env->ReleaseByteArrayElements(jObjAlpha, (jbyte *) lpSegMask, 0);
            }
            AndroidBitmap_unlockPixels(env, jObjfgImg);
            AndroidBitmap_unlockPixels(env, jObjbgImg);
        }
        return 0;
    }
}
    ...
    JNIEXPORT jint JNICALL
    Java_com_megvii_segjni_SegJni_nativeCreateImageBuffer(JNIEnv *env, jclass type, int width,
                                                          int height) {
